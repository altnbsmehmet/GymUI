...
public class PageController : Controller
{
    ...
    {   
        var membershipsResponse = await _httpClientService.GetAsync<GetMembershipsResponse>(new RequestModelBase { Route = $"membership/getall" });
    ...
    {
        var authorizationResponse = await _httpClientService.GetAsync<UserAuthorizationResponse>(new RequestModelBase { Route = $"user/authorizeuser" });

        if (authorizationResponse.IsSuccess && authorizationResponse.Role == "Member") {
            var membershipResponse = await _httpClientService.GetAsync<GetMembershipResponse>(new RequestModelBase { Route = $"membership/getbyid/{id}" });
    ...
    {
        var employeeResponse = await _httpClientService.GetAsync<GetEmployeesResponse>(new RequestModelBase { Route = $"employee/getall/trainer" });
    ...
    public async Task<IActionResult> GetProfilePage()
    {
        ...
        if (authorizationResponse.IsSuccess) {
            ...
            var subscriptionsResponse = await _httpClientService.GetAsync<GetSubscriptionsResponse>(new RequestModelBase { Route = $"subscription/getallbymemberid" });
    ...
    public async Task<IActionResult> GetAdminEmployeesPage()
    {
        var authorizationResponse = await _httpClientService.GetAsync<UserAuthorizationResponse>(new RequestModelBase { Route = $"user/authorizeuser" });
        if (authorizationResponse.IsSuccess && authorizationResponse.Role == "Admin") {
            var employeesResponse = await _httpClientService.GetAsync<GetEmployeesResponse>(new RequestModelBase { Route = $"employee/getall" });
