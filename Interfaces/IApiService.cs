...
public interface IApiService
{
    ...
    Task<T> PostAsync<T>(string controller, string endpoint, string id, [Body] object body, [Header("Authorization")] string? authorization = null, [HeaderCollection] IDictionary<string, string>? customHeaders = null);

    [Patch("/{controller}/{endpoint}")]
    Task<T> PatchAsync<T>(string controller, string endpoint, string id, [Body] object body, [Header("Authorization")] string? authorization = null, [HeaderCollection] IDictionary<string, string>? customHeaders = null);
